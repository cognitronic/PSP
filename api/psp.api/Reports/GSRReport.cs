...
namespace psp.api.Reports
{
    public class GSRReport
    {
        ...
        public GSR GetAmountToAudit(Site site, DateTime reportdate, string fromTime, string toTime)
        {
            ...
            {
                // Prime Shine Wash
                gsr.washLinkPrimeShine_count = wlData.primeshinewash > 0 ? wlData.primeshinewash : 0;
                gsr.washLinkPrimeShine_dollars = gsr.washLinkPrimeShine_count * (int)GSRMultiplier.PLUS_SEVEN;
                gsr.washLinkTotalPrimeShine_dollars = gsr.washLinkPrimeShine_count * (int)GSRMultiplier.PLUS_SEVEN;

                // Protex Wash
                gsr.washLinkProtex_count = wlData.protexwash > 0 ? wlData.protexwash : 0;
                gsr.washLinkProtex_dollars = gsr.washLinkProtex_count * (int)GSRMultiplier.PLUS_TEN;
                gsr.washLinkTotalProtex_dollars = gsr.washLinkProtex_count * (int)GSRMultiplier.PLUS_TEN;

                // Premier Wash
                gsr.washLinkPremier_count = wlData.premierwash > 0 ? wlData.premierwash : 0;
                gsr.washLinkPremier_dollars = gsr.washLinkPremier_count * (int)GSRMultiplier.PLUS_TWELVE;
                gsr.washLinkTotalPremier_dollars = gsr.washLinkPremier_count * (int)GSRMultiplier.PLUS_TWELVE;

                // Tire Shine
                gsr.washLinkTireGloss_count = wlData.tireshine > 0 ? wlData.tireshine : 0;
                gsr.washLinkTireGloss_dollars = gsr.washLinkTireGloss_count * (int)GSRMultiplier.PLUS_TWO;
                gsr.washLinkTotalTireGloss_dollars = gsr.washLinkTireGloss_count * (int)GSRMultiplier.PLUS_TWO;

                //Plus+
                gsr.washLinkPlusPlus_count = wlData.plusplus > 0 ? wlData.plusplus : 0;
                gsr.washLinkPlusPlus_dollars = gsr.washLinkPlusPlus_count * (int)GSRMultiplier.PLUS_THREE;
                gsr.washLinkTotalPlusPlus_dollars = gsr.washLinkPlusPlus_count * (int)GSRMultiplier.PLUS_THREE;

                // RainX
                gsr.washLinkRainX_count = wlData.rainx > 0 ? wlData.rainx : 0;
                gsr.washLinkRainX_dollars = gsr.washLinkRainX_count * (int)GSRMultiplier.PLUS_TWO;
                gsr.washLinkTotalRainX_dollars = gsr.washLinkRainX_count * (int)GSRMultiplier.PLUS_TWO;
                ...
                foreach (var item in swData)
                {
                    switch (item.objid)
                    {
                        ...
                        case "606590": // Fleet PSX
                            ...
                            gsr.primeShineFleet_count += int.Parse(item.total);
                            gsr.primeShineFleet_dollars += gsr.sitewatchFleetPsx_dollars;
                            break;
                        ...
                        case "49500138": // Unlimited UCW
                            ...
                            gsr.primeShineUnlimited_dollars += gsr.sitewatchUnlimitedUcw_dollars;
                            break;
                        ...
                        case "49501448": // Unlimited PSX Wash
                            ...
                            gsr.primeShineUnlimited_count += int.Parse(item.total);
                            gsr.primeShineUnlimited_dollars += gsr.sitewatchUnlimitedPsx_dollars;
                            break;
                        ...
                        case "49501535": // Fleet Protex
                            ...
                            gsr.protexFleet_count += int.Parse(item.total);
                            gsr.protexFleet_dollars += gsr.sitewatchFleetProtex_dollars;
                            break;
                        case "49501616": // PSX Fleet Protex
                            ...
                            gsr.protexFleet_count += int.Parse(item.total);
                            gsr.protexFleet_dollars += gsr.sitewatchPsxFleetProtex_dollars;
                            break;
                        case "49501531": // Unlimited Protex Wash
                            ...
                            gsr.protexUnlimited_count += int.Parse(item.total);
                            gsr.protexUnlimited_dollars += gsr.sitewatchUnlimitedProtex_dollars;
                            break;
                        case "608644": // Unlimited Protex W/TG Wash
                            ...
                            gsr.protexUnlimited_count += int.Parse(item.total);
                            gsr.protexUnlimited_dollars += gsr.sitewatchUnlimitedProtexWithTireGloss_dollars;
                            break;
                        ...
                        case "608917": // Unlimited Premier
                            ...
                            gsr.premierUnlimited_dollars += gsr.sitewatchUnlimitedPremier_dollars;
                            break;
                        case "608918": // Unlimited Premier W/TG
                            ...
                            gsr.premierUnlimited_count += int.Parse(item.total);
                            gsr.premierUnlimited_dollars += gsr.sitewatchUnlimitedPremierWithTireGloss_dollars;
                            break;
                        case "608817": // Fleet Premier
                            ...
                            gsr.premierFleet_count += int.Parse(item.total);
                            gsr.premierFleet_dollars += gsr.sitewatchFleetPremier_dollars;
                            break;
                        case "608832": // PSX Fleet Premier
                            ...
                            gsr.premierFleet_count += int.Parse(item.total);
                            gsr.premierFleet_dollars += gsr.sitewatchPsxFleetPremier_dollars;
                            break;
                        case "608771": // PSX Fleet Premier No Gloss
                            ...
                            gsr.premierFleet_count += int.Parse(item.total);
                            gsr.premierFleet_dollars += gsr.sitewatchPsxFleetPremierNoGloss_dollars;
                            break;
                        ...
                        case "609015": // Unlimited PSX With Plus+
                            ...
                            gsr.primeShineUnlimited_count += int.Parse(item.total);
                            gsr.primeShineUnlimited_dollars += gsr.sitewatchUnlimitedPsxWithPlusPlus_dollars;
                            break;
                        case "609014": // Unlimited PSX With RainX
                            ...
                            gsr.primeShineUnlimited_count += int.Parse(item.total);
                            gsr.primeShineUnlimited_dollars += gsr.sitewatchUnlimitedPsxWithRainX_dollars;
                            break;
                        case "609019": // Unlimited Protex With Plus+
                            ...
                            gsr.protexUnlimited_count += int.Parse(item.total);
                            gsr.protexUnlimited_dollars += gsr.sitewatchUnlimitedProtexWithPlusPlus_dollars;
                            break;
                        ...
                        case "609020": // Unlimited Premier With RainX
                            ...
                            gsr.premierUnlimited_count += int.Parse(item.total);
                            gsr.premierUnlimited_dollars += gsr.sitewatchUnlimitedPremierWithRainX_dollars;
                            break;
            ...
            gsr.sitewatchProtexWash_count = gsr.sitewatchTotalProtex_count;

            gsr.sitewatchTotalProtex_count = gsr.sitewatchProtexWash_count - (gsr.protexFleet_count + gsr.protexUnlimited_count + gsr.sitewatchProtexRewash_count);

            gsr.washLinkTotalProtex_count = gsr.washLinkProtex_count - (gsr.protexFleet_count + gsr.protexUnlimited_count + gsr.sitewatchProtexRewash_count);
            ...
            gsr.sitewatchProtexWash_dollars += gsr.sitewatchProtexWash_count * (int)GSRMultiplier.PLUS_TEN;
            ...
            gsr.sitewatchTotalProtex_dollars = gsr.sitewatchProtexWash_dollars + (gsr.protexFleet_dollars + gsr.protexUnlimited_dollars + gsr.sitewatchProtexRewash_dollars);
            gsr.washLinkTotalProtex_dollars = gsr.washLinkProtex_dollars + (gsr.protexFleet_dollars + gsr.protexUnlimited_dollars + gsr.sitewatchProtexRewash_dollars);

            gsr.totalProtex_diff = gsr.sitewatchTotalProtex_dollars - gsr.washLinkTotalProtex_dollars;
            ...
            gsr.sitewatchPremierWash_count = gsr.sitewatchTotalPremier_count;

            gsr.sitewatchTotalPremier_count = gsr.sitewatchPremierWash_count - (gsr.premierFleet_count + gsr.premierUnlimited_count + gsr.sitewatchPremierRewash_count);

            gsr.washLinkTotalPremier_count = gsr.washLinkPremier_count - (gsr.premierFleet_count + gsr.premierUnlimited_count + gsr.sitewatchPremierRewash_count);
            ...
            gsr.sitewatchPremierWash_dollars += gsr.sitewatchPremierWash_count * (int)GSRMultiplier.PLUS_TWELVE;
            ...
            gsr.sitewatchTotalPremier_dollars = gsr.sitewatchPremierWash_dollars + (gsr.premierFleet_dollars + gsr.premierUnlimited_dollars + gsr.sitewatchPremierRewash_dollars);
            gsr.washLinkTotalPremier_dollars = gsr.washLinkPremier_dollars + (gsr.premierFleet_dollars + gsr.premierUnlimited_dollars + gsr.sitewatchPremierRewash_dollars);

            gsr.totalPremier_diff = gsr.sitewatchTotalPremier_dollars - gsr.washLinkTotalPremier_dollars;
            ...
            gsr.sitewatchTotalPrimeShine_count = (gsr.sitewatchPrimeShine_count +
                ...
                gsr.sitewatchUnlimitedUcw_count) - (gsr.sitewatchProtexWash_count + gsr.sitewatchPremierWash_count);

            gsr.sitewatchPrimeShine_count = gsr.sitewatchTotalPrimeShine_count;

            gsr.sitewatchTotalPrimeShine_count = gsr.sitewatchPrimeShine_count - (gsr.primeShineFleet_count + gsr.primeShineUnlimited_count + gsr.sitewatchPrimeShineRewash_count);

            gsr.washLinkTotalPrimeShine_count = gsr.washLinkPrimeShine_count - (gsr.primeShineFleet_count + gsr.primeShineUnlimited_count + gsr.sitewatchPrimeShineRewash_count);



            // Prime Shine Total Dollars

            gsr.sitewatchPrimeShine_dollars = gsr.sitewatchPrimeShine_count * (int)GSRMultiplier.PLUS_SEVEN;
            ...
            gsr.sitewatchTotalPrimeShine_dollars = gsr.sitewatchPrimeShine_dollars + (gsr.primeShineFleet_dollars + gsr.primeShineUnlimited_dollars + gsr.sitewatchPrimeShineRewash_dollars);
            gsr.washLinkTotalPrimeShine_dollars = gsr.washLinkPrimeShine_dollars + (gsr.primeShineFleet_dollars + gsr.primeShineUnlimited_dollars + gsr.sitewatchPrimeShineRewash_dollars);

            gsr.totalPrimeShine_diff = gsr.sitewatchTotalPrimeShine_dollars - gsr.washLinkTotalPrimeShine_dollars;
            ...
            gsr.sitewatchTireGloss_count = gsr.sitewatchTotalTireGloss_count;

            gsr.sitewatchTotalTireGloss_count = gsr.sitewatchTireGloss_count - (gsr.sitewatchFleetTireGloss_count + gsr.sitewatchUnlimitedTireGloss_count + gsr.sitewatchReapplyTireGloss_count);
            gsr.washLinkTotalTireGloss_count = gsr.washLinkTireGloss_count - (gsr.sitewatchFleetTireGloss_count + gsr.sitewatchUnlimitedTireGloss_count + gsr.sitewatchReapplyTireGloss_count);

            gsr.sitewatchTireGloss_dollars += gsr.sitewatchTireGloss_count * (int)GSRMultiplier.PLUS_TWO;
            ...
            gsr.sitewatchTotalTireGloss_dollars = gsr.sitewatchTireGloss_dollars + (gsr.sitewatchReapplyTireGloss_dollars + gsr.sitewatchUnlimitedTireGloss_dollars + gsr.sitewatchFleetTireGloss_dollars);
            gsr.washLinkTotalTireGloss_dollars = gsr.washLinkTireGloss_dollars + (gsr.sitewatchReapplyTireGloss_dollars + gsr.sitewatchUnlimitedTireGloss_dollars + gsr.sitewatchFleetTireGloss_dollars);

            gsr.totalTireGloss_diff = gsr.sitewatchTotalTireGloss_dollars - gsr.washLinkTotalTireGloss_dollars;
            ...
            gsr.sitewatchRainX_count = gsr.sitewatchTotalRainX_count;

            gsr.sitewatchTotalRainX_count = gsr.sitewatchRainX_count - (gsr.sitewatchFleetRainX_count + gsr.sitewatchUnlimitedRainX_count + gsr.sitewatchReapplyRainX_count);
            gsr.washLinkTotalRainX_count = gsr.washLinkRainX_count - (gsr.sitewatchFleetRainX_count + gsr.sitewatchUnlimitedRainX_count + gsr.sitewatchReapplyRainX_count);

            gsr.sitewatchRainX_dollars += gsr.sitewatchRainX_count * (int)GSRMultiplier.PLUS_TWO;
            ...
            gsr.sitewatchTotalRainX_dollars = gsr.sitewatchRainX_dollars + (gsr.sitewatchReapplyRainX_dollars + gsr.sitewatchUnlimitedRainX_dollars + gsr.sitewatchFleetRainX_dollars);
            gsr.washLinkTotalRainX_dollars = gsr.washLinkRainX_dollars + (gsr.sitewatchReapplyRainX_dollars + gsr.sitewatchUnlimitedRainX_dollars + gsr.sitewatchFleetRainX_dollars);

            gsr.totalRainX_diff = gsr.sitewatchTotalRainX_dollars - gsr.washLinkTotalRainX_dollars;
            ...
            gsr.sitewatchPlusPlus_count = gsr.sitewatchTotalPlusPlus_count;

            gsr.sitewatchTotalPlusPlus_count = gsr.sitewatchPlusPlus_count - (gsr.sitewatchFleetPlusPlus_count + gsr.sitewatchUnlimitedPlusPlus_count + gsr.sitewatchReapplyPlusPlus_count);
            gsr.washLinkTotalPlusPlus_count = gsr.washLinkPlusPlus_count - (gsr.sitewatchFleetPlusPlus_count + gsr.sitewatchUnlimitedPlusPlus_count + gsr.sitewatchReapplyPlusPlus_count);

            gsr.sitewatchPlusPlus_dollars += gsr.sitewatchPlusPlus_count * (int)GSRMultiplier.PLUS_THREE;
            ...
            gsr.sitewatchTotalPlusPlus_dollars = gsr.sitewatchPlusPlus_dollars + (gsr.sitewatchReapplyPlusPlus_dollars + gsr.sitewatchUnlimitedPlusPlus_dollars + gsr.sitewatchFleetPlusPlus_dollars);
            gsr.washLinkTotalPlusPlus_dollars = gsr.washLinkPlusPlus_dollars + (gsr.sitewatchReapplyPlusPlus_dollars + gsr.sitewatchUnlimitedPlusPlus_dollars + gsr.sitewatchFleetPlusPlus_dollars);

            gsr.totalPlusPlus_diff = gsr.sitewatchTotalPlusPlus_dollars - gsr.washLinkTotalPlusPlus_dollars;
            ...
            gsr.sitewatchTotalWashes_dollars = gsr.sitewatchTotalPrimeShine_dollars +
                gsr.sitewatchTotalProtex_dollars +
                gsr.sitewatchTotalPremier_dollars +
                gsr.sitewatchTotalTireGloss_dollars +
                gsr.sitewatchTotalRainX_dollars +
                gsr.sitewatchTotalPlusPlus_dollars;

            gsr.washLinkTotalWashes_dollars = gsr.washLinkTotalPrimeShine_dollars +
                gsr.washLinkTotalProtex_dollars +
                gsr.washLinkTotalPremier_dollars +
                gsr.washLinkTotalTireGloss_dollars +
                gsr.washLinkTotalRainX_dollars +
                gsr.washLinkTotalPlusPlus_dollars;

            gsr.totalWashes_diff = gsr.sitewatchTotalWashes_dollars - gsr.washLinkTotalWashes_dollars;
            ...
            // Total Wash Service
            gsr.sitewatchTotalWashServices = gsr.sitewatchTotalWashes_dollars + gsr.impulseItems + gsr.webConnect;
            gsr.washLinkTotalWashServices = gsr.washLinkTotalWashes_dollars + gsr.impulseItems + gsr.webConnect;
            gsr.totalWashServicesDiff = gsr.sitewatchTotalWashServices - gsr.washLinkTotalWashServices;

            // Total To Account For
            gsr.sitewatchTotalToAccountFor = gsr.sitewatchTotalWashServices +
                 gsr.machineSales +
                gsr.pumpCodeWash +
                gsr.prePaids -
            ...
            gsr.totalToAccountForDiff = gsr.sitewatchTotalToAccountFor - gsr.washLinkTotalToAccountFor;
            ...
            // Total Over
            gsr.sitewatchTotalOverUnder_dollars = ((gsr.cashDeposit + gsr.creditCards) + gsr.sitewatchTotalToAccountFor) * (int)GSRMultiplier.NEGATIVE_ONE;
            gsr.washLinkTotalOverUnder_dollars = ((gsr.cashDeposit + gsr.creditCards) + gsr.washLinkTotalToAccountFor) * (int)GSRMultiplier.NEGATIVE_ONE;

            gsr.totalOverUnder_diff = gsr.sitewatchTotalOverUnder_dollars - gsr.washLinkTotalOverUnder_dollars;
            ...
            gsr.amountToAudit = gsr.sitewatchTotalOverUnder_dollars;
            return gsr;
        }
